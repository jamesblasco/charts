...
abstract class BaseCartesianRenderer<D> extends BaseSeriesRenderer<D>
  ...
  void onAttach(BaseRenderChart<D> chart) {
    ...
    this.chart = chart as CartesianRenderChart<D>;
  }
  ...
  void configureMeasureAxes(List<MutableSeries<D>> seriesList) {
    for (final series in seriesList) {
      ...
      final domainAxis =
          series.getAttr(domainAxisKey) as MutableAxisElement<D>?;
      final domainFn = series.domainFn;
      ...
      final measureAxis =
          series.getAttr(measureAxisKey) as MutableAxisElement<num>?;
      if (measureAxis == null) {
        continue;
      }
  ...
  void addMeasureValuesFor(
    ...
    MutableAxisElement<num> measureAxis,
    int startIndex,
    int endIndex,
  ) {
