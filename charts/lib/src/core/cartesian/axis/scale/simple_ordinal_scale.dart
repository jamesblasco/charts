...
/// by [[]].
class SimpleOrdinalScaleElement implements OrdinalScaleElement {
  SimpleOrdinalScaleElement() : _domain = OrdinalScaleDomainInfo();

  SimpleOrdinalScaleElement._copy(SimpleOrdinalScaleElement other)
  ...
  void setViewportSettings(double viewportScale, double viewportTranslate) {
    ...
    if (_isVertical) {
      _viewportTranslate = max(
        min(-(rangeWidth * (1.0 - viewportScale)), viewportTranslate),
        0,
      );
    } else {
      _viewportTranslate =
  ...
  String? get viewportStartingDomain {
    ...
    if (_isVertical) {
      ...
      final index = (-(rangeWidth + _viewportTranslate) / _cachedStepSizePixels)
              .ceil()
              .toInt() -
          1;
      ...
      return _domain.getDomainAtIndex(
        (-_viewportTranslate / _cachedStepSizePixels).ceil().toInt(),
      );
    }
  }
  ...
  @override
  SimpleOrdinalScaleElement copy() => SimpleOrdinalScaleElement._copy(this);
  ...
  void _recalculateScale() {
    ...
    switch (rangeBandConfig.type) {
      ...
      case RangeBandType.fixedPixelSpaceFromStep:
        final spaceInPixels = rangeBandConfig.size.toDouble();
