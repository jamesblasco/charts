...
@immutable
abstract class Scale<D> extends Equatable {
  const Scale();
  ScaleElement<D> createElement();
}
...
abstract class ScaleElement<D> {
  ...
  MutableScaleElement<D> copy();
}

/// Mutable extension of the [ScaleElement] definition.
///
...
abstract class MutableScaleElement<D> extends ScaleElement<D> {
  /// Reset the domain for this [ScaleElement].
  void resetDomain();

  /// Reset the viewport settings for this [ScaleElement].
  void resetViewportSettings();

  /// Add [domainValue] to this [ScaleElement]'s domain.
  ///
