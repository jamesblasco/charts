...
typedef BehaviorCreator = ChartBehaviorState<D> Function<D>();

abstract class BaseRenderChart<D> {
  BaseRenderChart({LayoutConfig? layoutConfig})
  ...
  final _behaviorRoleMap = <String, ChartBehaviorState<D>>{};
  final _behaviorStack = <ChartBehaviorState<D>>[];

  final _behaviorTappableMap = <String, ChartBehaviorState<D>>{};
  ...
  /// chart.
  ChartBehaviorState<D> createBehavior(BehaviorCreator creator) => creator<D>();
  ...
  void addBehavior(ChartBehaviorState<D> behavior) {
  ...
  bool removeBehavior(ChartBehaviorState<D>? behavior) {
    if (behavior == null) {
      return false;
    }
  ...
  /// via [addBehavior].
  void registerTappable(ChartBehaviorState<D> behavior) {
    final role = behavior.role;
  ...
  void unregisterTappable(ChartBehaviorState<D> behavior) {
    final role = behavior.role;
  ...
  List<ChartBehaviorState<D>> get behaviors =>
  ...
  void drawInternal(
    ...
    seriesList = seriesList.map(MutableSeries<D>.clone).toList();
  ...
  void paint(ChartCanvas canvas) {
    ...
    for (final view in _layoutManager.paintOrderedViews) {
